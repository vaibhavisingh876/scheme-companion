...
export const PATTERNS = {
  ...
  special: {
    ...
    childFocused: /child|children|school|mid.?day|meal|poshan|anganwadi|creche|juvenile/i,
    deathAssistance: /accidental death|natural death|funeral assistance|antim.*anudan|kabir.*anthyesthi|death.*assistance|death.*grant/i,
    // 🔧 Broadened: was a fixed list of specific award phrases and missed
    // things like "Nari Shakti Puraskar" (which showed up unfiltered for a
    // widow-pension seeker in real traffic). \bpuraskar\b / \baward\b at
    // name level naturally skips false positives like "...scholarships are
    // *awarded* on the basis of..." since \b fails inside "awarded".
    award: /national youth award|national award.*excel|bravery award|pradhan mantri rashtriya bal puraskar|guru shishya|repertory grant|cultural.*grant.*perform|\bpuraskar\b|\baward\b/i,
    // 🆕 Freedom-fighter pensions kept surfacing for unrelated welfare
    // requests (e.g. shown to a widow asking about widow pension). Same
    // treatment as exServicemen below — require explicit mention.
    freedomFighter: /freedom fighter|swatantrata sainik|independence movement (fighter|pensioner)/i,
    // 🆕 Schemes scoped to orphans / wards of martyred police-armed-forces
    // personnel kept surfacing for ordinary students (e.g. AICTE-Swanath,
    // PM Scholarship for Wards of Police Personnel shown to a regular
    // General-category student). These aren't caste-reserved so the caste
    // hard-reject never caught them — separate pattern, same treatment.
    nicheBeneficiaryWard: /\borphan(s)?\b|\bmartyred\b|ward(s)?\s*of\b.{0,25}(police|armed\s*forces|defence|paramilitary)|children\s*of\s*(the\s*)?martyred|covid.?19?\s*orphan/i,
    overseas: /indian community welfare fund|icwf|overseas indian|pravasi bharatiya|non.?resident indian|\bnri\b/i,
    financial: /national savings certificate|nsc.*viii|post office.*income|monthly income scheme|pomis|nps vatsalya|pension.*minor|mahila samman savings|public provident fund|\bppf\b|sukanya samriddhi|handloom.*mudra|sfurti|khadi.*loan|swatantrata sainik/i,
    banking: /pradhan mantri jan dhan|pmjdy|jan dhan yojana|pradhan mantri jeevan jyoti bima|pmjjby|pradhan mantri suraksha bima|pmsby|atal pension yojana|kisan vikas patra|post office savings|recurring deposit|fixed deposit.*scheme/i,

    // 🔧 FIX: this array used to live inline inside scoringEngine.js's
    // isReservedScheme() and had a broken literal — `/\bsc/st\b|.../i`
    // contains an unescaped "/" inside the regex body, which terminates the
    // literal early and leaves "st" to be parsed as regex FLAGS. "t" is not
    // a valid flag, so this actually threw "SyntaxError: Invalid regular
    // expression flags" the moment the module was loaded (verified with
    // `node --check`). That means the whole caste hard-reject path — and
    // possibly the whole scoringEngine module — was never running as
    // intended. Fixed by escaping the slash, and centralized here so
    // scoringEngine.js and any future caller share one definition instead
    // of re-declaring it inline.
    reservedCategory: [
      /\bsc\b|scheduled\s*caste|\bdalit\b/i,
      /\bst\b|scheduled\s*tribe|\btribal\b|\badivasi\b/i,
      /\bobc\b|other\s*backward\s*class|backward\s*class/i,
      /\bminority\b|\bmuslim\b|\bsikh\b|\bchristian\b|\bjain\b|\bbuddhist\b|\bparsi\b/i,
      /\bcharmakar\b|\bdhangar\b|\bsahariya\b|\bvjnt\b|\bsbc\b|\bdnt\b|\bnomadic\b|\bdenotified\b/i,
      /de-notified|semi-nomadic|\bprimitive\b|\bvulnerable\b/i,
      /\bsc\s*\/\s*st\b|\bsc\s*&\s*st\b/i,
      /\bfor\s*sc\b|\bfor\s*st\b|\bfor\s*obc\b|\bfor\s*minority\b/i,
      /scheduled\s*caste|scheduled\s*tribe|backward\s*classes/i,
    ],
  },
};